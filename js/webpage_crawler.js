...
var webcrawler = {
	...
	setStartChatHandler: function() {
		if(this.dom['inputField']) {
			this.dom['inputField'].onfocus = function() {
				webcrawler.startChat();
				// Reset the onfocus event on first call:
				webcrawler.dom['inputField'].onfocus = '';
			}			
		}
	},
	...
	initEmoticons: function() {
		...
		for(var i=0; i<this.emoticonCodes.length; i++) {
			...
			this.DOMbuffer = this.DOMbuffer
						+ '<a href="javascript:webcrawler.insertText(\''
	...
	initColorCodes: function() {
		if(this.dom['colorCodesContainer']) {
			...
			for(var i=0; i<this.colorCodes.length; i++) {
				this.DOMbuffer = this.DOMbuffer
					+ '<a href="javascript:webcrawler.setFontColor(\''
					+ this.colorCodes[i]
	...
	loadFlashInterface: function() {
		if(this.dom['flashInterfaceContainer']) {
			this.updateDOM(
				...
				'<object id="webcrawlerFlashInterface" style="position:absolute; left:-100px;" '
				+'classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" '
				+'codebase="'
				+ window.location.protocol
				+'//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" '
				+'height="1" width="1">'
				+'<param name="flashvars" value="bridgeName=webcrawler"/>'
				+'<param name="src" value="'+this.dirs['flash']+'FABridge.swf"/>'
				+'<embed name="webcrawlerFlashInterface" pluginspage="'
				+ window.location.protocol
				+'//www.macromedia.com/go/getflashplayer" '
				+'src="'+this.dirs['flash']+'FABridge.swf" height="1" width="1" flashvars="bridgeName=webcrawler"/>'
				+'</object>'
			);
			FABridge.addInitializationCallback('webcrawler', this.flashInterfaceLoadCompleteHandler);
		}
	},
	
	flashInterfaceLoadCompleteHandler: function() {
		webcrawler.initializeFlashInterface();
	},
	...
	socketConnect: function() {
		if(!this.socketIsConnected) {
			try {
				if(!this.socket && FABridge.webcrawler) {
					this.socket = FABridge.webcrawler.create('flash.net.XMLSocket');
	...
	socketConnectHandler: function(event) {
		webcrawler.socketIsConnected = true;
		// setTimeout is needed to avoid calling the flash interface recursively:
		setTimeout('webcrawler.socketRegister()', 0);
	},

	socketCloseHandler: function(event) {
		webcrawler.socketIsConnected = false;
		if(webcrawler.socket) {
			clearTimeout(webcrawler.timer);
			webcrawler.updateChat(null);
		}
	},
	
	socketDataHandler: function(event) {
		webcrawler.socketUpdate(event.getData());
	},
	...
	socketIOErrorHandler: function(event) {
		...
		setTimeout('webcrawler.addChatBotMessageToChatList(\'/error SocketIO\')', 0);
		setTimeout('webcrawler.updateChatlistView()', 1);
	},

	socketSecurityErrorHandler: function(event) {
		// setTimeout is needed to avoid calling the flash interface recursively (e.g. sound on new messages):
		setTimeout('webcrawler.addChatBotMessageToChatList(\'/error SocketSecurity\')', 0);
		setTimeout('webcrawler.updateChatlistView()', 1);
	},
		...
		if(!isNaN(volume)) {
			...
			try {
				if(!this.soundTransform) {
					this.soundTransform = FABridge.webcrawler.create('flash.media.SoundTransform');					
				}
	...
	soundLoadCompleteHandler: function(event) {
		...
		for(var key in webcrawler.soundFiles) {
			// Get the sound key by matching the sound URL with the sound filename:
			if((new RegExp(webcrawler.soundFiles[key])).test(sound.getUrl())) {
				// Add the loaded sound to the sounds list:
				webcrawler.sounds[key] = sound;
			}
		}
	},
	...
	soundIOErrorHandler: function(event) {
		// setTimeout is needed to avoid calling the flash interface recursively (e.g. sound on new messages):
		setTimeout('webcrawler.addChatBotMessageToChatList(\'/error SoundIO\')', 0);
		setTimeout('webcrawler.updateChatlistView()', 1);
	},
	...
	makeRequest: function(url, method, data) {
		webcrawler.setStatus('On');
		webcrawler.retryTimer = setTimeout("webcrawler.updateChat(null); webcrawler.setStatus('Alert');", this.retryTimerDelay);
		try {
			var identifier;
			if(data) {
			...
			this.getHttpRequest(identifier).onreadystatechange = function() {
				try {
					webcrawler.handleResponse(identifier);
				} catch(e) {
					try {
						clearTimeout(webcrawler.timer);
					} catch(e) {
					...
					try {
						if(data) {
							webcrawler.addChatBotMessageToChatList('/error ConnectionTimeout');
							webcrawler.setStatus('Alert');
							webcrawler.updateChatlistView();
						}
					} catch(e) {
						//alert(e);
					}
					try {				
						webcrawler.timer = setTimeout('webcrawler.updateChat(null);', webcrawler.timerRate);
					} catch(e) {
						//alert(e);
					}
				}
			...
			this.timer = setTimeout('webcrawler.updateChat(null);', this.timerRate);
		}
	},
	...
	handleResponse: function(identifier) {
		if (this.getHttpRequest(identifier).readyState == 4) {
			if (this.getHttpRequest(identifier).status == 200) {
				clearTimeout(webcrawler.retryTimer);
				var xmlDoc = this.getHttpRequest(identifier).responseXML;
				webcrawler.setStatus('Off');
			} else {
		...
		if(this.chatStarted) {
			...
			if(this.socketIsConnected) {
				...
				if(this.socketServerEnabled && !this.socketReconnectTimer) {
					...
					this.socketReconnectTimer = setTimeout('webcrawler.socketConnect();', 60000);
				}
			}
			this.timer = setTimeout('webcrawler.updateChat(null);', timeout);			
		}
	},
	...
	getUserNodeString: function(userID, userName, userRole) {
		if(this.userNodeString && userID == this.userID) {
			...
			var str	= '<div id="'
					...
					+ '"><a href="javascript:webcrawler.toggleUserMenu(\''
	...
	getUserNodeStringItems: function(encodedUserName, userID, isInline) {
		...
		if(encodedUserName != this.encodedUserName) {
			menu 	= '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/msg '
					+ encodedUserName
					+ ' \');">'
					+ this.lang['userMenuSendPrivateMessage']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/describe '
					+ encodedUserName
					+ ' \');">'
					+ this.lang['userMenuDescribe']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/query '
					+ encodedUserName
					+ '\');">'
					...
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/ignore '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuIgnore']
					+ '</a></li>';
			if (isInline) {
				menu	+= '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/invite '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuInvite']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/uninvite '
					+ encodedUserName
					+ '\');">'
					+ this.lang['userMenuUninvite']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/whereis '
					+ encodedUserName
					+ '\');">'
			...
			if(this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/kick '
						+ encodedUserName
						+ ' \');">'
						+ this.lang['userMenuKick']
						+ '</a></li>'
						+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/whois '
						+ encodedUserName
						+ '\');">'
			...
			menu 	= '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/quit\');">'
					+ this.lang['userMenuLogout']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/who\');">'
					+ this.lang['userMenuWho']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/ignore\');">'
					+ this.lang['userMenuIgnoreList']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/list\');">'
					+ this.lang['userMenuList']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/action \');">'
					+ this.lang['userMenuAction']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/roll \');">'
					+ this.lang['userMenuRoll']
					+ '</a></li>'
					+ '<li><a href="javascript:webcrawler.insertMessageWrapper(\'/nick \');">'
					+ this.lang['userMenuNick']
					+ '</a></li>';
			if(this.userRole == 1 || this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/join\');">'
						+ this.lang['userMenuEnterPrivateRoom']
						+ '</a></li>';
				if(this.userRole == 2 || this.userRole == 3) {
					menu	+= '<li><a href="javascript:webcrawler.sendMessageWrapper(\'/bans\');">'
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		return	'<div id="'
				...
				+ ' dir="'
				+ this.baseDirection
				+ '" onclick="webcrawler.insertText(this.firstChild.nodeValue);">'
				+ userName
				+ '</span>'
				+ colon
	...
	getMessageDocumentID: function(messageID) {
		return ((messageID === null) ? 'webcrawler_lm_'+(this.localID++) : 'webcrawler_m_'+messageID);
	},
	...
	getUserMenuDocumentID: function(userID) {
		return 'webcrawler_um_'+userID;
	},
	
	getInlineUserMenuDocumentID: function(menuID, index) {
		return 'webcrawler_ium_'+menuID+'_'+index;
	},
	...
	getDeletionLink: function(messageID, userID, userRole, channelID) {
		if(messageID !== null && this.isAllowedToDeleteMessage(messageID, userID, userRole, channelID)) {
			...
			return	'<a class="delete" title="'
					...
					+ '" href="javascript:webcrawler.deleteMessage('
					+ messageID
	...
	blinkOnNewMessage: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		if(this.settings['blink'] && this.lastID && !this.channelSwitch && userID != this.userID) {
			...
			this.blinkInterval = setInterval(
				'webcrawler.blinkUpdate(\''+this.addSlashes(this.decodeSpecialChars(userName))+'\')',
	...
	replaceCommandInvite: function(textParts) {
		var inviteText = this.lang['invite']
							...
							.replace(
								/%s/,
								'<a href="javascript:webcrawler.sendMessageWrapper(\'/join '
	...
	replaceCommandBans: function(textParts) {
		...
		for(var i=0; i<users.length; i++) {
			listUsers.push(
				'<a href="javascript:webcrawler.sendMessageWrapper(\'/unban '
	...
	getInlineUserMenu: function(users) {
		...
		for(var i=0; i<users.length; i++) {
			...
			menu	+= '<a href="javascript:webcrawler.toggleUserMenu(\''
	...
	replaceBBCodeCallback: function(str, p1, p2, p3) {
		...
		if(!webcrawler.inArray(webcrawler.bbCodeTags, p1)) {
			return str;
		}	
		// Avoid invalid XHTML (unclosed tags):
		if(webcrawler.containsUnclosedTags(p3)) {
			return str;
		}			
		switch(p1) {
			case 'color':
				return webcrawler.replaceBBCodeColor(p3, p2);
			case 'url':
				return webcrawler.replaceBBCodeUrl(p3, p2);
			case 'img':
				return webcrawler.replaceBBCodeImage(p3);
			case 'quote':
				return webcrawler.replaceBBCodeQuote(p3, p2);
			case 'code':
				return webcrawler.replaceBBCodeCode(p3);
			case 'u':
				return webcrawler.replaceBBCodeUnderline(p3);
			default:
				return webcrawler.replaceCustomBBCode(p1, p2, p3);
		}	
	},
	...
	replaceBBCodeColor: function(content, attribute) {
		if(this.settings['bbCodeColors']) {
			...
			if(!attribute || !this.inArray(webcrawler.colorCodes, attribute))
				return content;								
	...
	replaceBBCodeImage: function(url) {
		if(this.settings['bbCodeImages']) {
			...
			return	'<a href="'
					...
					+'px;" src="'
					+url
					+'" alt="" onload="webcrawler.updateChatlistView();"/></a>';
		}
		return url;
	},
	...
	replaceEmoticonsCallback: function(str, p1, p2, p3) {
		...
		if(p2) {
			var index = webcrawler.arraySearch(p2, webcrawler.emoticonCodes);							
			return 	webcrawler.replaceEmoticons(p1)
				+	'<img src="'
				+	webcrawler.dirs['emoticons']
				+	webcrawler.emoticonFiles[index]
				+	'" alt="'
				+	p2
				+	'" />'
				+ 	webcrawler.replaceEmoticons(p3);
		}
		return str;
	},
