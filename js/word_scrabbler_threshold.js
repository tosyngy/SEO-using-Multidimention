...
// Overrides client-side functionality for the logs view:
	...
	webscrabbler.logsMonitorMode = null;
	webscrabbler.logsLastID = null;
	webscrabbler.logsCommand = null;

	webscrabbler.startChatUpdate = function() {
	...
	webscrabbler.updateChat = function(paramString) {
	...
	webscrabbler.sendMessage = function() {
		this.getLogs();	
	}
	
	webscrabbler.getLogs = function() {
	...
	webscrabbler.getLogsCommand = function() {
		if(!this.logsCommand) {
	...
	webscrabbler.onNewMessage = function(dateObject, userID, userName, userRoleClass, messageID, messageText, channelID, ip) {
	...
	webscrabbler.logout = function() {
		clearTimeout(this.timer);
		this.makeRequest(this.ajaxURL,'POST','logout=true');
	}

	webscrabbler.switchLanguage = function(langCode) {
		window.location.search = '?view=logs&lang='+langCode;
	}

	webscrabbler.setChatUpdateTimer = function() {
		clearTimeout(this.timer);
		var timeout;
		if(this.socketIsConnected && this.logsLastID && this.lastID == this.logsLastID) {
			...
			if(this.socketServerEnabled && !this.socketReconnectTimer) {
				...
				this.socketReconnectTimer = setTimeout('webscrabbler.socketConnect();', 60000);
			}
		}
		this.timer = setTimeout('webscrabbler.updateChat(null);', timeout);
	}
	
	webscrabbler.socketUpdate = function(data) {
		if(this.logsMonitorMode) {
